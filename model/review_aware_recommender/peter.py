...
def generate_square_subsequent_mask(total_len):
    ...
    mask = mask == 0  # lower -> False(available); others True(mask)
    return mask
...
class PETER(nn.Module):
    def __init__(self, config):
        ...
        self.pos_encoder = PositionalEncoding(config['embedding_size'], config['dropout_prob'])  # emsize: word embedding size
        encoder_layers = TransformerEncoderLayer(config['embedding_size'], config['nhead'], config['hidden_size'], config['dropout_prob'])  # nhid: dim_feedforward, one basic layer, including multi-head attention and FFN
        self.transformer_encoder = TransformerEncoder(encoder_layers, config['nlayers'])  # loop over the one above
        ...
        self.word_embeddings = nn.Embedding(config['token_num'], config['embedding_size'])  # word emb
        self.hidden2token = nn.Linear(config['embedding_size'], config['token_num'])  # the probability of each word
        self.recommender = MLP(config['embedding_size'])

        self.ui_len = 2
        self.src_len = config['src_len']
        self.pad_idx = config['pad_idx']
        self.emsize = config['embedding_size']
        if config['peter_mask']:
            self.attn_mask = generate_peter_mask(config['src_len'], config['tgt_len'])
        else:
            self.attn_mask = generate_square_subsequent_mask(config['src_len'] + config['tgt_len'])
    ...
    def forward(self, user, item, text, seq_prediction=True, context_prediction=True, rating_prediction=True):
        ...
        # see nn.Multihead Attention for attn_mask and key_padding_mask
