...
class DERM_MLP(nn.Module):
    def __init__(self, config):
        super(DERM_MLP, self).__init__()
        ...
        self.transfer_layer = MLPLayers([config['embedding_size']*2] + config['mlp_hidden_size'], config['dropout_prob'])
    ...
    def get_mid_state(self, user, item):
        ...
        mid_state = self.transfer_layer(torch.cat((u_emb, i_emb), dim=1))
        return mid_state
